...
```bash
lambda-activity$ sam build --use-container
```
...
```bash
lambda-activity$ sam logs -n StockCheckerFunction --stack-name lambda-activity --tail
```
...
```bash
lambda-activity$ pip install -r tests/requirements.txt --user
# unit test
lambda-activity$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
lambda-activity$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name lambda-activity
```
